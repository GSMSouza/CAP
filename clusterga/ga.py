...
from clusterga.utils import Random, plot_history
...
def ga(samples, fitness_function="CalinskiHarabaz", gen=100, size_pop=40, p_crossover=0.5, p_mutation=0.1,
    ...
    if type(mutation_operator) is MutationInterface:
        ...
        try:
            mutation = dic.mutation[mutation_operator](samples, max_groups, rand)
        except KeyError:
    ...
    if type(crossover_operator) is CrossoverInterface:
        ...
        try:
            crossover = dic.crossover[crossover_operator](rand)
        except KeyError:
    ...
    if type(population_operator) is PopulationInterface:
        ...
        try:
            population = dic.population[population_operator](samples, rand)
        except KeyError:
    ...
    if type(selection_operator) is SelectionInterface:
        ...
        try:
            selection = dic.selection[selection_operator](population, rand)
        except KeyError:
